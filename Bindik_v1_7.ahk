...
IniRead, Zif1, %SaveFile%, Time, Zif1
IniRead, Zif2, %SaveFile%, Time, Zif2
...
Gui, Add, Edit, Center number x135 y70 w100 h20 vZif1 hwndhEdit1 gEdit1, Zif1
GuiControl, , Zif1, %Zif1%
...
Gui, Add, Edit, Center number x135 y130 w100 h20 vZif2 hwndhEdit2 gEdit2, Zif2
GuiControl, , Zif2, %Zif2%
Gui, font, cBlack
...
;-------------------------------------Block:Restore-------------------------------------------------
Restore: ; Проверяет есть ли ini файл в диске C, чтобы брать от туда инфу. Если его нет, оно его создаёт.
{
	IfNotExist, %SaveFile%
		{
		IniWrite, %space%, %SaveFile%, Section, MyHotkey1
		IniWrite, %space%, %SaveFile%, Section, MyHotkey2
		IniWrite, 0, %SaveFile%, Time, Zif1
		IniWrite, 0, %SaveFile%, Time, Zif2
		IniWrite, 1, %SaveFile%, Settings, Ind
		IniWrite, %space%, %SaveFile%, Settings, NameWin
...
Edit1:
	...
	if Zif1 is space
	{
		...
		if (Zif1 ~= "^00"){
			...
			Zif1 := RegExReplace(%Kov%Zif1%Kov%, "^0")
			GuiControl, , Zif1, %Zif1%
			sendinput, {end}
			Rezz1 := Zif1 * 1000
		}
	IniWrite, %Zif1%, %SaveFile%, Time, Zif1
    Return

Edit2:
	GuiControlGet, Zif2, , Zif2
	if Zif2 is space
	{
		...
		if (Zif2 ~= "^00"){
			...
			Zif2 := RegExReplace(%Kov%Zif2%Kov%, "^0")
			GuiControl, , Zif2, %Zif2%
			sendinput, {end}
			Rezz2 := Zif2 * 1000
		}
	IniWrite, %Zif2%, %SaveFile%, Time, Zif2
    Return
...
Lock:
	if l = 1 ; Если заблокировано
	{
		...
		GuiControl, -Disabled, Zif1, % Zif1
		If Zif1 = 0
			GuiControl, , Zif1,
		GuiControl,-Disabled, Zif2, % Zif2
		If Zif2 = 0
			GuiControl, , Zif2,
		l := 0
	}
